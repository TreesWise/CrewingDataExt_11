...
# # CSV and pickle file paths in Azure Storage
...
# # Helper function to download files from Azure Blob Storage
# def download_blob(blob_name: str) -> bytes:
#     blob_client = container_client.get_blob_client(blob_name)
#     blob_data = blob_client.download_blob().readall()
#     return blob_data

# # Initialize CSV in Azure if it doesn't exist
# def initialize_csv_in_azure():
#     try:
#         # Try to download the CSV to check if it exists
#         download_blob(CSV_BLOB_NAME)
#     except:
#         # If the CSV doesn't exist, create an empty one
#         df = pd.DataFrame(columns=["user_id", "pdf_name", "doc_id", "output_file_name"])
#         csv_data = df.to_csv(index=False).encode('utf-8')
#         upload_blob(csv_data, CSV_BLOB_NAME)

# # Save user data to a pickle file in Azure
...
# # Load user data from a pickle file in Azure
...
# # Update the CSV file stored in Azure Blob Storage
...
# # Validate if the given user_id and doc_id pair exists in the Azure CSV
...
# # Check if the doc_id already exists in the Azure CSV file
...
# # Generate a unique doc_id that doesn't exist in the Azure CSV
...
#     # Save initial metadata and content in-memory
#     user_data[user_id] = {
...
#     # Save the initial state to a pickle file in Azure
#     save_user_data_to_pickle(user_id, doc_id)

#     # Update the CSV file in Azure
#     update_csv_in_azure(user_id, pdf_name, doc_id, output_file_name)

#     # Run both the timer and the text extraction task simultaneously
#     background_tasks.add_task(run_background_tasks, user_id, doc_id, file_content)

#     # Generate the dynamic URL for the user with doc_id included
...
#     """Starts a 15-minute timer for a given user."""
#     await asyncio.sleep(5 * 60)  # 15-minute timer
#     user_data[user_id]['ready'] = True
#     save_user_data_to_pickle(user_id ,doc_id)  # Save state after the timer ends

# def extract_pdf_content_task(user_id: str, doc_id: str, file_content: bytes):
#     """Background task to extract PDF content."""
#     file_stream = BytesIO(file_content)
#     extracted_text = cv_json(file_stream)  # Extract text using cv_json function    
#     user_data[user_id]["pdf_text"] = extracted_text
#     save_user_data_to_pickle(user_id, doc_id)  # Save the state once extraction is done
...
#         raise HTTPException(status_code=500, detail="Extraction process is not completed yet.")
    
#     # return {"user_id": user_id, "doc_id": doc_id, "pdf_content": user_info["pdf_text"]}
